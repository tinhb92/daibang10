...
## Core Principle 5: "The Convergent AMM Yield Doctrine — Pendle V2 Pools vs Boros Orderbooks"
> *"In an orderbook CLOB like Boros, resting capital earns 0.00% unless filled. In a Pendle V2 AMM, capital captures multi-stream cash flows with mathematically guaranteed zero impermanent loss at maturity."*

### 1. Structural Comparison: Boros vs Pendle V2 Pools
- **Boros (Arbitrum CLOB):** Pure orderbook for borrowing and lending yield. Depositing liquidity earns **0.00% pool LP yield**. Fills depend entirely on counterparty flow; resting orders require constant manual or bot maintenance.
- **Pendle V2 AMM Pools (Robinhood Chain):** Autonomous Market Maker pools where LPs deposit into $PT / SY$ pools.

### 2. The 4 Cash Flow Streams of Pendle Pools
1. **Underlying Yield:** Continuous staking or interest rate generated by the accounting asset (e.g. `104.20%` on sNUKE, `52.37%` on SHROOM).
2. **AMM Swap Fees:** Continuous trading volume fee capture paid by PT/YT/SY swappers (e.g. `27.18%` on sNUKE, `16.44%` on SHROOM).
3. **Protocol Incentive Emissions:** Direct PENDLE token liquidity rewards distributed to LPs (e.g. `13.33%` on sNUKE, `138.89%` on SGOV).
4. **PT Fixed Accretion:** Natural pull-to-par ($PT \to SY$ at 1:1) providing guaranteed capital accretion from the initial PT discount.

### 3. The Convergent Zero-Impermanent-Loss Invariant
- Unlike standard Uniswap pools where token price divergence causes permanent impermanent loss, Pendle V2 pools pair $PT$ with $SY$ (the exact same underlying asset).
- At expiration timestamp $T$, $1 \text{ PT} = 1 \text{ SY}$.
- Holding an LP position to maturity mathematically guarantees **Zero Impermanent Loss at Maturity** ($IL \to 0$).

### 4. Asymmetric Fat Pitch Criteria for AMM Pools
- **Criterion 1 (Ajit Jain Barrier):** Only enter pools where $\text{DTE} \ge 7.0$ days. Avoid terminal rebalancing friction inside the 7-day cliff.
- **Criterion 2 (Minimum Depth Floor):** Only enter pools with $\text{TVL} \ge \$5,000 \text{ USD}$ to prevent illiquid exit traps (e.g. avoid `microduck` at $1.1k TVL).
- **Criterion 3 (Overwhelming Premium):** Aggregated APY must exceed the desk's hurdle rate (e.g. `sNUKE` at `147.70%` APY).
